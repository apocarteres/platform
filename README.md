...
Модули: `platform-bom`, `platform-service-parent`, `platform-arch-rules`, `platform-persistence`, `platform-time`, `platform-web-errors`, `platform-api-version`; пакеты `packages/project-conventions`, `packages/http`, `packages/routing`, `packages/modal`, `packages/action`, `packages/app-update`.
