---
id: PLATFORM-PROPOSAL-SHARED-CORE
type: decision
status: proposed
...
# Общее ядро для zavpn и clanlog: анализ и предложение

Дата анализа: 2026-09-06. Документ для обсуждения; после принятия решений
инварианты переносятся в требования платформы, а сам документ получает
статус `accepted` или `rejected`.
...
| `platform-notification` | Порты `NotificationChannel`, `TemplateRenderer`, `NotificationOutbox`; email SMTP и Resend; локализация через `MessageSource`; шаблоны с табличной вёрсткой и инлайн-стилями; текстовая часть; брендовые токены как параметры проекта; `drain()` как команда | новое, первый потребитель по решению |
...
1. **Формат метаданных документов.** YAML front matter (zavpn) или bold-строки (clanlog)?
   Рекомендация: front matter. Типизирован, проверяется с тестами, поддерживает `depends-on`, `related`, `superseded-by`. От clanlog взять каталоги по областям, статус `cancelled` с обязательным разделом причины, `INDEX.md` в каждом каталоге вместо `README.md`. Миграция ~110 тикетов clanlog скриптом.
   Зависит: 5.3.
   Ответ: —
...
3. **Где рендерятся письма.** Сервер (шаблонизатор в Java-библиотеке) или сборка из Angular (предмет `RES-001` в clanlog)?
   ...
   Зависит: 5.1 `platform-notification`, пять тикетов по уведомлениям в обоих проектах.
   Ответ: —

4. **Первый потребитель уведомлений.** zavpn (три тикета, вопросы сняты) или clanlog (FEAT-003/004, заблокированы RES-001)?
   Зависит: порядок этапа 5 в разделе 7.
   Ответ: —
...
5. **`platform-notification`.** Первый компонент, спроектированный один раз для двух потребителей.
6. **Angular-пакеты** после стабилизации серверной части.

## 8. Персональные данные

Уведомления и рассылки означают обработку email и IP-адресов пользователей, в zavpn также данных о трафике. Это персональные данные в смысле 152-ФЗ. До реализации рассылок из ЛК админа в требованиях должны появиться: основание обработки, согласие на рассылку и его отзыв, сроки хранения истории уведомлений и журналов доступа. В `auth-and-mail.md` и `cabinet.md` zavpn и в `common-lk.md` clanlog этого сейчас нет.
