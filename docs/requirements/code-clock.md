...
6. <a id="REQ-CODE-CLOCK-009"></a> **REQ-CODE-CLOCK-009** — Подстановка в шаблонной строке — код, а не литерал: обращение к часам внутри `${…}` является нарушением наравне с обращением вне строки. Текст самого шаблона остаётся литералом.
7. <a id="REQ-CODE-CLOCK-006"></a> **REQ-CODE-CLOCK-006** — Обращения внутри строковых литералов и комментариев нарушением не считаются.
...
9. <a id="REQ-CODE-CLOCK-007"></a> **REQ-CODE-CLOCK-007** — Существующие обращения фиксируются ограничителем `.conventions/baseline.json` отдельным разделом правила. Первичное засеивание разрешено без ключей, последующий рост требует `--allow-growth`.
