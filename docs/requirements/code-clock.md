...
4. <a id="REQ-CODE-CLOCK-004"></a> **REQ-CODE-CLOCK-004** — Время получают через интерфейс часов: в рабочем коде он реализован системными часами, в тестах возвращает заданное значение. Средство для конкретного языка задают [порт часов Java](java-clock.md) и [порт часов TypeScript](typescript-clock.md).
