...
9. <a id="REQ-CODE-CLOCK-007"></a> **REQ-CODE-CLOCK-007** — Существующие обращения фиксируются храповиком `.conventions/baseline.json` отдельным разделом правила. Первичное засеивание разрешено без ключей, последующий рост требует `--allow-growth`.
