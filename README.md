...
Модули: `platform-bom`, `platform-service-parent`, `platform-arch-rules`, `platform-persistence`, `platform-time`, `platform-web-errors`, `platform-api-version`, `platform-rate-limit`, `platform-auth`, `platform-notifications`, `platform-support`; пакеты `packages/project-conventions`, `packages/http`, `packages/routing`, `packages/modal`, `packages/action`, `packages/app-update`, `packages/auth`, `packages/client-journal`, `packages/support`, `packages/notifications`.
