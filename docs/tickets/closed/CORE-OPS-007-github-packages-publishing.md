...
related: ADR-0001, REQ-PUBLISHING, RUN-GITHUB-PACKAGES, CORE-DATA-001
...
- Первый публикуемый starter — `platform-persistence`; его перенос идёт по [CORE-DATA-001](CORE-DATA-001-platform-persistence.md), здесь он нужен как реальный артефакт для проверки канала публикации.
