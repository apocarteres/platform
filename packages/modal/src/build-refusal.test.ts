...
async function compile(template: string): Promise<string> {
  ...
  try {
    await writeFile(path.join(probe, 'consumer.ts'), [
      ...
      "import { ApoModal, ESCAPE_IGNORED } from '../src/modal';",
      '',
      `@Component({ selector: 'app-consumer', standalone: true, imports: [ApoModal], template: '${template}' })`,
...
describe('сборка потребителя со строгими шаблонами', () => {
  it('отказывает модальному окну без решения об Escape и собирает окно с решением', async () => {
    const forgot = await compile('<div apoModal>забыли</div>');
    expect(forgot).toContain('NG8008');
    expect(forgot).toContain("Required input 'apoModalEscape' from directive ApoModal must be specified");
    expect(await compile('<div apoModal [apoModalEscape]="ignored">решили</div>')).toBe('');
  }, 120_000);
});
